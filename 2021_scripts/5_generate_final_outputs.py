...
def main(paperlist_name, session_order_name, start_page, output_dir):
    ...
    with open(os.path.join(output_dir, 'publications_ISMIR2021.txt'), 'wb') as f:
        html = template.render(context)
        f.write(html.encode('utf-8'))

    with open(os.path.join(output_dir, '2021.json'), 'w', encoding='utf-8') as fp:
        json.dump(publications, fp, indent=4, ensure_ascii=False)

    with open(os.path.join(output_dir, '2021_internal.json'), 'w', encoding='utf-8') as fp:
